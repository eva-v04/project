...
export type PatternMatchJSON = {
    loc: LocationJSON,
    //add
    encl_loc?: LocationJSON, //τοποθεσία caller
    uncertainties?: Array<{
        text: string,
...
export class PatternMatcher {
    ...
    private findAccessPathPatternMatches(p: AccessPathPattern, moduleFilter?: ModuleFilter, write?: boolean): AccessPathPatternMatches {
       ...
       // if (logger.isDebugEnabled())
            for (const level of confidenceLevels) {
                for (const [e, [aps, encl]] of res[level]) { // Πρέπει να κάνεις extract το 'encl' εδώ
                    for (const ap of aps) {
                        const callerLine = (encl && encl.loc) ? encl.loc.start.line : "top-level";
                        // DEBUG
                        logger.error(`DEBUG: Pattern matched at line ${e.loc?.start.line} inside caller function starting at line ${callerLine}. Matched access path: ${ap.toString()}`);
                    }
                }
            }
            return res;
    ...
    findDetectionPatternMatches(d: DetectionPattern, diagnostics?: AnalysisDiagnostics, moduleFilter?: ModuleFilter): Array<DetectionPatternMatch> {
        //DEBUG
        logger.error(`!!! ENTERING DETECTION MATCHING for pattern: ${d.toString()}`);
        const timer = new Timer();
        this.findEscapingAccessPathsToExternal();
        const res: Array<DetectionPatternMatch> = [];
        if (d instanceof ImportDetectionPattern) { //αν το μοτίβο δείχνει ΄ότι είναι Import
            const sub = this.findAccessPathPatternMatches(d.ap, moduleFilter);
            for (const level of confidenceLevels) //confidenceLevels: how certain we are that the access path matches the pattern, "high" or "low"
            ...
            if (ms.length > 0) {
                ...
                for (const m of ms) {
                    const match: PatternMatchJSON = {
                        // προσθέτει μόνο exp και uncertainties !λείπει encl
                        loc: locs.makeLocString(m.exp.loc), //add
                        encl_loc: m.encl ? locs.makeLocString(m.encl.loc) : undefined, //μετατρέπω θέση caller σε string, για να εμφανίζεται στο output JSON
                        uncertainties: []
                    };
                    //δεν εμφανίζεται το debug print
                    logger.error(`DEBUG: Found API usage at ${match.loc} inside caller ${match.encl_loc}`);
                    matches.push(match);
