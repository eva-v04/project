...
import { SQLITE3_OPEN_CREATE, SQLITE3_OPEN_MEMORY, SQLITE3_OPEN_READONLY, SQLITE3_OPEN_READWRITE, SQLITE_BLOB, SQLITE_FLOAT, SQLITE_INTEGER, SQLITE_NULL, SQLITE_TEXT } from "./constants.ts";
import { readCstr, toCString, unwrap } from "./util.ts";
import { type RestBindParameters, Statement, STATEMENTS_TO_DB } from "./statement.ts";
...
export type Transaction<T extends (...args: any[]) => void> = ((...args: Parameters<T>) => ReturnType<T>) & {
  /** BEGIN */
...
export const BIG_MAX = BigInt(Number.MAX_SAFE_INTEGER);
/**
...
export class Database {
  ...
  sql<T extends Record<string, any> = Record<string, any>>(strings: TemplateStringsArray, ...parameters: RestBindParameters): T[] {
   ...
   * ```
   */
  function(name: string, fn: CallableFunction, options?: FunctionOptions): void {
    ...
    const cb = new Deno.UnsafeCallback({
      ...
      try {
        ...
        const buf = new TextEncoder().encode(err instanceof Error ? err.message : String(err));
...
export const controllers = new WeakMap(); // Return the database's cached transaction controller, or create a new one
export const getController = (db: Database) => {
...
export const wrapTransaction = <T extends (...args: any[]) => void,>(fn: T, db: Database, {
  begin,
  commit,
  rollback,
  savepoint,
  release,
  rollbackTo
}: any) => function sqliteTransaction(...args: Parameters<T>): ReturnType<T> {
  const {
    apply
  } = Function.prototype;
