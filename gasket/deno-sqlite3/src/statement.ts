...
import { SQLITE3_DONE, SQLITE3_ROW, SQLITE_BLOB, SQLITE_FLOAT, SQLITE_INTEGER, SQLITE_TEXT } from "./constants.ts";
const {
  sqlite3_prepare_v2,
  sqlite3_reset,
...
export type BindValue = number | string | symbol | bigint | boolean | null | undefined | Date | Uint8Array | BindValue[] | {
...
export const emptyStringBuffer = new Uint8Array(1);
export const statementFinalizer = new FinalizationRegistry((ptr: Deno.PointerValue) => {
...
function getColumn(handle: Deno.PointerValue, i: number, int64: boolean, parseJson: boolean): any {
...
export class Statement<TStatement extends object = Record<string, any>> {
  ...
  constructor(public db: Database, sql: string) {
    ...
    unwrap(sqlite3_prepare_v2(db.unsafeHandle, cString, cString.byteLength, pHandle, null), db.unsafeHandle);
      ...
      case "object":
        {
          if (param === null) {
            ...
            unwrap(sqlite3_bind_blob(this.#handle, i + 1, param.byteLength === 0 ? emptyStringBuffer : param as BufferSource, param.byteLength, null));
  ...
  #valuesNoArgs<T extends Array<unknown>>(): T[] {
    ...
    const getRowArray = new Function("getColumn", `
      return function(h, int64, parseJson) {
        return [${Array.from({
      length: columnCount
    }).map((_, i) => `getColumn(h, ${i}, int64, parseJson)`).join(", ")}];
      };
      `)(getColumn);
  ...
  #valuesWithArgs<T extends Array<unknown>>(...params: RestBindParameters): T[] {
    const handle = this.#handle;
    this.#begin();
    this.#bindAll(params);
    const columnCount = sqlite3_column_count(handle);
  ...
  getRowObject(): (h: Deno.PointerValue, int64: boolean, parseJson: boolean) => any {
    if (!this.#rowObjectFn || !this.#unsafeConcurrency) {
      ...
      const getRowObject = new Function("getColumn", `
        return function(h, int64, parseJson) {
          return {
            ${columnNames.map((name, i) => `"${name}": getColumn(h, ${i}, int64, parseJson)`).join(",\n")}
          };
        };
        `)(getColumn);
